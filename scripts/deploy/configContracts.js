...
async function main() {
  ...
  const opProvider = "https://mainnet.optimism.io";
  const avalancheProvider = "https://nd-118-315-546.p2pify.com/048dd2e7493f4804ffed70b2acfffe8b/ext/bc/C/rpc";
  const baseProvider = hre.config.networks.base.url;

  const provider = new ethers.providers.JsonRpcProvider(baseProvider);
  ...
  const wethSet = await fiberRouter.connect(wallet).setWETH(wethBase);
  ...
  const wethForgeSet = await multiswapForge.connect(wallet).setWETH(wethBase);
  // Wait for the transaction receipt
  ...
  const targetAllowed = await fundManager.connect(wallet).allowTarget(foundryBase, ethereumChainID, foundryEthereum);
  // Wait for the transaction receipt
  ...
  // Call allowTarget on FundManager with specified addresses
  const targetAllowed2 = await fundManager.connect(wallet).allowTarget(foundryBase, binanceChainID, foundryBinance);
  // Wait for the transaction receipt
  ...
  // Call allowTarget on FundManager with specified addresses
  const targetAllowed3 = await fundManager.connect(wallet).allowTarget(foundryBase, arbitrumChainID, foundryArbitrum);
  // Wait for the transaction receipt
  ...
  // Call allowTarget on FundManager with specified addresses
  const targetAllowed4 = await fundManager.connect(wallet).allowTarget(foundryBase, optimismChainID, foundryOptimism);
  // Wait for the transaction receipt
  ...
  const foundryAdded = await fundManager.connect(wallet).addFoundryAsset(foundryBase);
  // Wait for the transaction receipt
  ...
  const forgeFoundryAdded = await forgeManager.connect(wallet).addFoundryAsset(foundryBase);
  // Wait for the transaction receipt
